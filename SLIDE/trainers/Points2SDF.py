...
class Trainer(BaseTrainer):
    ...
    def update(self,  cfg, input_points, near_net, far_net, epoch, step,gt_inner, gt_outer,*args, **kwargs):
        ...
        if (dims == 3):
            ...
            n_inner_near = gradient(near_net(gt_inner), gt_inner)/torch.norm(gradient(near_net(gt_inner), gt_inner), dim = -1).view(inner_size, 1)
            n_outer_near = gradient(near_net(gt_outer), gt_outer)/torch.norm(gradient(near_net(gt_outer), gt_outer), dim = -1).view(outer_size, 1)
            n_inner_far = gradient(far_net(gt_inner), gt_inner)/torch.norm(gradient(far_net(gt_inner), gt_inner), dim = -1).view(inner_size, 1)
            n_outer_far = gradient(far_net(gt_outer), gt_outer)/torch.norm(gradient(far_net(gt_outer), gt_outer), dim = -1).view(outer_size, 1)
            pot_cut_inner = self.net(gt_inner)
            pot_cut_outer = self.net(gt_outer)
            ...
            #outer_loss = eta2(self.net(outer), 0.1)
