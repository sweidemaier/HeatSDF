...
new_net_path = "/home/weidemaier/PDE Net/NFGP/logs/NeuralSDFs2_2025-Mar-12-09-26-09"
...
def render_sdf_quad(render_path, contour_path, gradient_path,  P0, P1, P2, res=100):
    ...
    if render_path is not None:
        norm = colors.TwoSlopeNorm(vmin=vmin, vmax=vmax, vcenter=5)
        plt.clf()
    ...
    if contour_path is not None:
        plt.clf()
        norm = colors.TwoSlopeNorm(vmin=vmin, vmax=vmax, vcenter=(vmin+vmax)/2)
        plt.imshow(img, cmap="bwr", norm=norm)
        plt.axis("off")
        # cs = plt.contourf(X,-Y,img, levels=np.linspace(-0.1,0.1,11), cmap="seismic", extend="both")
        # cs.changed()
        plt.contour(img, levels=12, colors='k', linestyles="solid", linewidths=0.3)
