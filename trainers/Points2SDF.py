...
class Trainer(BaseTrainer):
    ...
    def update(self,  cfg, input_points, near_net, far_net, epoch, step,gt_inner, gt_outer,*args, **kwargs):
        if (epoch == 0):
            if(step == 1):
                print("init")
        ...
        outer_radius = 1.5#(domain_bound - 0.1)
        ...
        def eta(x, delta= 0.005, shift = 0):
            vec = (1/4)*(x/(delta+shift) + 2*torch.torch.ones_like(x))*(x/(delta+shift) - torch.ones_like(x))**2
            vec = torch.where(x <= -(delta+shift)*torch.ones_like(x),  torch.ones_like(x), vec)
            vec = torch.where(x > (delta+shift)*torch.ones_like(x), torch.zeros_like(x), vec)
            ...
            #else:
            #    return (1/4)*(x/delta + 2*torch.torch.ones_like(x))*(x/delta - torch.ones_like(x))**2
        ...
        if (dims == 3):
            ...
            #    i += 1
            #vec = tens(vec)
            path = cfg.input.point_path
           
            if (path == "/home/weidemaier/PDE Net/NFGP/armadillo2.csv"):
                inner = inner + tens([0.15,0.3,0.25]).reshape(1,3).repeat(500, 1)
            elif(path == "/home/weidemaier/PDE Net/NFGP/actaeon.csv"):
                inner = inner + tens([-0.2,-0.1,-0.15]).reshape(1,3).repeat(500, 1)
            elif(path == "/home/weidemaier/New Network/NFGP/lightbulb.csv"):
                inner = inner + tens([0,0,-0.85]).reshape(1,3).repeat(500, 1)
            elif(path == "/home/weidemaier/PDE Net/NFGP/recon benchmark set/lordquas.csv"):
                inner = inner + tens([-0.15,-0.2,-0.2]).reshape(1,3).repeat(500, 1)
            elif(path == "/home/weidemaier/PDE Net/NFGP/bunny.csv"):
                inner = inner + tens([0.,-0.3,0]).reshape(1,3).repeat(500, 1)


            #inner = inner + tens([0.15,0.3,0.25]).reshape(1,3).repeat(500, 1)#+ vec.reshape(500,3)#tens([0,translate,0]).reshape(1,3).repeat(500, 1)
            ...
            #    u_far = far_net(xyz)
            #    u_near = near_net(xyz)
            #    #print(eta(torch.norm(grad_u_near, dim = -1) - gamma, shift = 0).view(bs, 1))
            #    return eta(torch.norm(grad_u_near, dim = -1) - gamma, shift = gamma).view(bs, 1)*u_far + (1 - eta(torch.norm(grad_u_near, dim = -1) - gamma, shift = gamma)).view(bs,1)*u_near
            #n = gradient(u(xyz), xyz)/torch.norm(gradient(u(xyz), xyz), dim = -1).view(bs, 1)
            
            grad_u_near = gradient(near_net(xyz), xyz)
            grad_u_far = gradient(far_net(xyz), xyz)
            gamma = 500
            n = eta(torch.norm(grad_u_near, dim = -1) - gamma, shift = gamma).view(bs, 1)*grad_u_far + (1-eta(torch.norm(grad_u_near, dim = -1) - gamma, shift = gamma).view(bs, 1))*grad_u_near
            n = n/torch.norm(n, dim = -1).view(bs, 1)
            ...
            grad_u_near_inner = gradient(near_net(gt_inner), gt_inner)
            grad_u_near_outer = gradient(near_net(gt_outer), gt_outer)
            ...
            #pot_cut_inner = near_net(gt_inner)
            #pot_cut_outer = near_net(gt_outer)
            #n_inner = torch.where(pot_cut_inner > cut_off_param*torch.ones(inner_size, 1).cuda(), n_inner_near, n_inner_far)
            #n_outer = torch.where(pot_cut_outer > cut_off_param*torch.ones(outer_size, 1).cuda(), n_outer_near, n_outer_far)
            
            #print(n_inner)
            #print(torch.norm(grad_inner + n_inner, dim = -1).view(inner_size, 1).mean())
            
            n_inner = eta(torch.norm(grad_u_near_inner, dim = -1) - gamma, shift = gamma).view(inner_size, 1)*grad_far_inner + (1-eta(torch.norm(grad_u_near_inner, dim = -1) - gamma, shift = gamma).view(inner_size, 1))*grad_near_inner
            n_inner = n_inner/torch.norm(n_inner, dim = -1).view(inner_size, 1)

            n_outer = eta(torch.norm(grad_u_near_outer, dim = -1) - gamma, shift = gamma).view(outer_size, 1)*grad_far_outer + (1-eta(torch.norm(grad_u_near_outer, dim = -1) - gamma, shift = gamma).view(outer_size, 1))*grad_near_outer
            n_outer = n_outer/torch.norm(n_outer, dim = -1).view(outer_size, 1)
            # gradient(u(inner), inner)/torch.norm(gradient(u(inner), inner), dim = -1).view(inner_size, 1)
            
            #n_outer = gradient(u(outer), outer)/torch.norm(gradient(u(outer), outer), dim = -1).view(outer_size, 1)
             
            inner_loss = torch.norm(grad_inner - n_inner, dim = -1).view(inner_size, 1) #eta2(-self.net(gt_inner), 0.1) #torch.norm(grad_inner + n_inner, dim = -1).view(inner_size, 1) #eta2(self.net(outer), 0.1) #((torch.arctan((-1)*(100)*self.net(outer)) + np.pi/2)/np.pi) #
            outer_loss = torch.norm(grad_outer + n_outer, dim = -1).view(outer_size, 1) #eta2(self.net(outer), 0.1) #torch.norm(grad_outer - n_outer, dim = -1).view(outer_size, 1) #eta2(-self.net(inner), 0.01) #
            #inner_loss = eta2(-self.net(inner), 0.01) 
            #outer_loss = eta2(self.net(outer), 0.01)
        ...
        loss = scale*factor[0]*(u_zero_squared.mean())+ factor[2]* bd_loss + factor[1]*normal_alignment.mean()  #+ 0.001*sing_fac*singular_hess.mean()#+ 3*sing_fac*singular_hess.mean()
    ...
    def validate(self, cfg, input_points, near_net, far_net, writer, epoch, *args, **kwargs):
        ...
        if (dims == 3):
            ...
            if (path == "/home/weidemaier/PDE Net/NFGP/armadillo2.csv"):
                ...
                inner = inner + tens([0,0,-0.85]).reshape(1,3).repeat(500, 1)
            elif(path == "/home/weidemaier/New Network/NFGP/lordquas.csv"):
                inner = inner + tens([0,0,-0.15]).reshape(1,3).repeat(500, 1)
            elif(path == "/home/weidemaier/PDE Net/NFGP/bunny.csv"):
                inner = inner + tens([0.,-0.3,0]).reshape(1,3).repeat(500, 1)
            #inner2 = inner + tens([-0.3,0.6,-0.2]).reshape(1,3).repeat(500, 1)
            #inner3 = inner + tens([-0.25,-0.7,0.3]).reshape(1,3).repeat(500, 1)
