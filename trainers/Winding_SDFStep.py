...
from trainers.helper import sample_points_from_box_midpoints, eta, beta
...
class Trainer(BaseTrainer):
    ...
    def update(self,  cfg, input_points, near_net, far_net,  kappa, inner_sample, outer_sample, epoch, step):
        if (epoch == 0):
            if(step == 1):
                print("init")
                return {}
        ...
        ### sort normals
        weight = eta(u, np.float32(cfg.input.parameters.delta))
        if(far_net != None):
            u_far = far_net(xyz)
        ...
        inner_loss = eta(-inner, np.float32(cfg.input.parameters.delta))
        outer_loss = eta(outer, np.float32(cfg.input.parameters.delta))
    ...
    def validate(self,  cfg, input_points, near_net, far_net,  kappa, inner_sample, outer_sample, epoch, writer):
        ###load settings
        domain_bound = cfg.input.parameters.domain_bound
